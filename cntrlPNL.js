(function () {
    ...
    tip.style = 'position:fixed;background:#1a1a1e;border:1px solid var(--accent,#0f0);color:#ccc;padding:6px 10px;font-size:10px;z-index:200000;visibility:hidden;pointer-events:none;max-width:180px;line-height:1.5;font-family:monospace;';
    ...
    const builder = (rawConfig, index) => {
        ...
        const buildColorPicker = (item, container) => {
            ...
            let opacity = 1.0, hue = 0;

            const {r:ir,g:ig,b:ib}=hexToRgb(currentHex);
            hue = rgbToHsl(ir,ig,ib).h;
            ...
            if (hasExtras) {
                ...
                if (hasBox) {
                    ...
                    const cw=document.createElement('div'); cw.style.cssText='position:relative;margin-bottom:2px;';
                    cw.append(canvas,boxHandle); extras.appendChild(cw);
                    const drawBox=()=>{ const w=canvas.offsetWidth||230,h=canvas.height; canvas.width=w; ctx=canvas.getContext('2d'); ctx.clearRect(0,0,w,h); ctx.fillStyle=`hsl(${hue},100%,50%)`; ctx.fillRect(0,0,w,h); const wg=ctx.createLinearGradient(0,0,w,0); wg.addColorStop(0,'rgba(255,255,255,1)'); wg.addColorStop(1,'rgba(255,255,255,0)'); ctx.fillStyle=wg; ctx.fillRect(0,0,w,h); const bg=ctx.createLinearGradient(0,0,0,h); bg.addColorStop(0,'rgba(0,0,0,0)'); bg.addColorStop(1,'rgba(0,0,0,1)'); ctx.fillStyle=bg; ctx.fillRect(0,0,w,h); };
                    const pickBox=e=>{ const r=canvas.getBoundingClientRect(),w=r.width,h=r.height; const x=Math.max(0,Math.min(w,e.clientX-r.left)),y=Math.max(0,Math.min(h,e.clientY-r.top)); boxHandle.style.left=x+'px'; boxHandle.style.top=y+'px'; const {r:rr,g:gg,b:bb}=hslToRgb(hue,(x/w)*100,100-(y/h)*50*(1+(1-x/w))); updateDOM(rgbToHex(rr,gg,bb)); proxy[item.key]=currentHex; };
                    canvas.addEventListener('mousedown',e=>{ drawBox(); pickBox(e); const mv=e2=>pickBox(e2); const up=()=>{window.removeEventListener('mousemove',mv);window.removeEventListener('mouseup',up);}; window.addEventListener('mousemove',mv); window.addEventListener('mouseup',up); });
                    requestAnimationFrame(drawBox);
                }

                if (hasSlider) {
                    const hr=document.createElement('div'); hr.className='pnl-color-hue-row';
                    swatchLg=document.createElement('div'); swatchLg.className='pnl-color-swatch-lg'; swatchLg.style.background=currentHex;
                    hueSlider=document.createElement('input'); hueSlider.type='range'; hueSlider.className='pnl-color-hue-slider'; hueSlider.min=0; hueSlider.max=360; hueSlider.step=1; hueSlider.value=Math.round(hue);
                    hueSlider.oninput=e=>{ hue=parseInt(e.target.value); const {r:rr,g:gg,b:bb}=hexToRgb(currentHex); const {s,l}=rgbToHsl(rr,gg,bb); const {r:nr,g:ng,b:nb}=hslToRgb(hue,s,l); updateDOM(rgbToHex(nr,ng,nb)); proxy[item.key]=currentHex; if(hasBox&&ctx)setTimeout(()=>{ const w=canvas.offsetWidth||230,h=canvas.height; canvas.width=w; ctx.fillStyle=`hsl(${hue},100%,50%)`; ctx.fillRect(0,0,w,h); const wg=ctx.createLinearGradient(0,0,w,0); wg.addColorStop(0,'rgba(255,255,255,1)'); wg.addColorStop(1,'rgba(255,255,255,0)'); ctx.fillStyle=wg; ctx.fillRect(0,0,w,h); const bg=ctx.createLinearGradient(0,0,0,h); bg.addColorStop(0,'rgba(0,0,0,0)'); bg.addColorStop(1,'rgba(0,0,0,1)'); ctx.fillStyle=bg; ctx.fillRect(0,0,w,h); },0); };
                    hr.append(swatchLg,hueSlider); extras.appendChild(hr);
                }

                if (hasOpacity) {
                    const or=document.createElement('div'); or.className='pnl-color-opacity-row';
                    ...
                    opGrad=document.createElement('div'); opGrad.className='pnl-color-opacity-grad'; opGrad.style.background=`linear-gradient(to right,transparent,${currentHex})`;
                    opRange=document.createElement('input'); opRange.type='range'; opRange.className='pnl-color-opacity-range'; opRange.min=0; opRange.max=1; opRange.step=0.01; opRange.value=1;
                    opNum=document.createElement('input'); opNum.className='pnl-opacity-num'; opNum.type='text'; opNum.value='1.00';
                    opRange.oninput=e=>{ opacity=parseFloat(e.target.value); opNum.value=opacity.toFixed(2); if(item.onOpacity)item.onOpacity(opacity); };
                    makeScrub(opNum,()=>opacity,v=>{ opacity=Math.max(0,Math.min(1,v)); opRange.value=opacity; opNum.value=opacity.toFixed(2); if(item.onOpacity)item.onOpacity(opacity); },0.01,0,1);
                    track.append(checker,opGrad,opRange); or.append(track,opNum); extras.appendChild(or);
                }
                ...
                if (hasRgb) {
                    ...
                    rgbEl.append(rf,gf,bf); extras.appendChild(rgbEl);
                }
    ...
    window.addEventListener('DOMContentLoaded', () => { if(window.pnlConfigs) window.pnlConfigs.forEach((c,i)=>builder(c,i)); });
})();
