// cntrlPNL ESM build
// Usage: import { cntrlPNL, pnlStates } from './cntrlPNL.esm.js'

export const pnlStates = [];

// pnlStates exported above
...
/* ── ESM export ─────────────────────────────────────────────────────── */
let _nextIndex = 1; // 0 reserved for settings panel convention

export function cntrlPNL(config, index) {
    if (index === undefined) {
        while (pnlStates[_nextIndex] !== undefined) _nextIndex++;
        index = _nextIndex++;
    }
    builder(normalize(config), index);
    return pnlStates[index];
}
